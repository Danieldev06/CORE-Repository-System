...
import { Search, Filter, Grid, List, Download, Bookmark, BookmarkCheck, ChevronLeft, ChevronRight, X, Loader2 } from 'lucide-react';
import { useApp } from '../context';
import { SCHOOLS, RESOURCE_TYPE_LABELS } from '../data';
  ...
  return (
    <div className="bg-white rounded-xl border border-navy-100 p-5 hover:border-navy-300 hover:shadow-md transition-all duration-150 flex flex-col group">
      ...
      <button onClick={() => navigate('resource-detail', { id: resource.id })} className="text-left flex-1">
        ...
        <p className="text-navy-400 text-xs leading-relaxed line-clamp-2 mb-3">{resource.description}</p>
      </button>
      <div className="border-t border-navy-50 pt-3 mt-auto space-y-2">
        ...
        {resource.courseCode && (
          <div className="text-[11px] text-navy-400">{resource.courseCode} · {resource.fileType} · {resource.fileSize}</div>
        )}
        <div className="flex items-center justify-between">
          <div className="text-[11px] text-navy-400 flex items-center gap-2">
            <span className="flex items-center gap-1"><Download size={10} /> {resource.downloads}</span>
          </div>
          <button
    ...
    <div className="flex items-center gap-4 px-4 py-3.5 border-b border-navy-50 hover:bg-navy-50 transition group">
      <div className="flex-1 min-w-0">
        <button onClick={() => navigate('resource-detail', { id: resource.id })} className="text-left">
          <div className="font-semibold text-navy-800 text-sm group-hover:text-navy-600 transition truncate">{resource.title}</div>
          <div className="text-[11px] text-navy-400 mt-0.5">{resource.author} · {resource.courseCode ?? ''} · {resource.academicYear} · {resource.fileType} {resource.fileSize}</div>
        </button>
      </div>
      <div className="hidden md:block shrink-0"><ResourceTypeBadge type={resource.type} /></div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button onClick={() => toggleBookmark(resource.id)} className={`p-1.5 rounded-lg transition ${bookmarked ? 'text-navy-700' : 'text-navy-300 hover:text-navy-600'}`}>
          {bookmarked ? <BookmarkCheck size={14} /> : <Bookmark size={14} />}
        </button>
        <button onClick={() => addDownload(resource.id)} className="flex items-center gap-1.5 px-2.5 py-1.5 bg-navy-800 hover:bg-navy-700 text-white text-xs font-semibold rounded-lg transition">
          <Download size={11} /> Download
        </button>
      </div>
    </div>
...
export default function RepositoryView() {
  ...
  const filtered = useMemo(() => {
    ...
    return res;
  }, [resources, search, selectedType, selectedSchool, selectedYear]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const paginated = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  const clearFilters = () => { setSelectedType(''); setSelectedSchool(''); setSelectedYear(''); setSearch(''); setPage(1); };
      ...
      <div className="p-6">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            placeholder="Search resources, courses, authors, or keywords..."
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-navy-400 hover:text-navy-600">
              <X size={15} />
            </button>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-5">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="text-xs text-navy-500">{filtered.length} resource{filtered.length !== 1 ? 's' : ''}</span>
            <div className="flex border border-navy-200 rounded-lg overflow-hidden">
              <button onClick={() => setViewMode('grid')} className={`p-1.5 transition ${viewMode === 'grid' ? 'bg-navy-800 text-white' : 'text-navy-400 hover:bg-navy-50'}`}>
                <Grid size={14} />
              </button>
              <button onClick={() => setViewMode('list')} className={`p-1.5 transition ${viewMode === 'list' ? 'bg-navy-800 text-white' : 'text-navy-400 hover:bg-navy-50'}`}>
                <List size={14} />
              </button>
            </div>
          </div>
        ...
        {paginated.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-xl border border-navy-100">
            ...
            <button onClick={clearFilters} className="mt-4 text-sm text-navy-600 hover:text-navy-800 font-medium">Clear all filters</button>
          </div>
        ) : viewMode === 'grid' ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {paginated.map(r => <ResourceGridCard key={r.id} resource={r} />)}
          </div>
        ) : (
          <div className="bg-white rounded-xl border border-navy-100 overflow-hidden">
            <div className="bg-navy-50 border-b border-navy-100 px-4 py-2.5 flex items-center gap-4">
              <span className="flex-1 text-xs font-semibold text-navy-600 uppercase tracking-wide">Resource</span>
              <span className="hidden md:block w-32 text-xs font-semibold text-navy-600 uppercase tracking-wide">Type</span>
              <span className="hidden lg:block w-16 text-xs font-semibold text-navy-600 uppercase tracking-wide">Downloads</span>
              <span className="w-24 text-xs font-semibold text-navy-600 uppercase tracking-wide text-right">Actions</span>
            </div>
            {paginated.map(r => <ResourceListRow key={r.id} resource={r} />)}
          </div>
        )}
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
              <button key={p} onClick={() => setPage(p)}
                className={`w-9 h-9 rounded-lg text-sm font-semibold transition ${p === page ? 'bg-navy-800 text-white' : 'border border-navy-200 text-navy-600 hover:bg-navy-50'}`}>
                {p}
              </button>
            ))}
